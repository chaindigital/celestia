...
CACHE_BACKUP_FILE = "peer_cache_mainnet_backup.json"
...
def get_peers():
    try:
        ...
        log(f"❌ Ошибка получения пиров: {e}")
        return []
...
def save_cache(cache):
    shutil.copyfile(CACHE_FILE, CACHE_BACKUP_FILE) if os.path.exists(CACHE_FILE) else None
...
def main():
    ...
    for pid in tqdm(peers, desc="Пиры"):
        ...
        geo["peer_id"] = pid
        new_cache[pid] = geo
        new_cache[pid]["updated_at"] = datetime.now(timezone.utc).isoformat()
        result.append(geo)
        time.sleep(0.3)
...
# === Добавление cron (каждые 5 минут) ===
